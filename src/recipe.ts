...
const recipeId: number = parseInt(params.get('id') as string)

const mainElement: HTMLElement = document.querySelector('main') as HTMLElement

const platesList = JSON.parse(localStorage.getItem('plates') || '[]')

const recipe = platesList.find((element: Recipe) => element.id === recipeId)

const isLoggedIn: boolean = JSON.parse(sessionStorage.getItem('isConnected') || 'false')
...
if (recipe === undefined) {
    ...
    <section class="flex flex-col gap-2">
            ...
            <section class="flex flex-col gap-2">
                ...
                <ul class='flex flex-col gap-2'>
                    ${recipe.ingredients.map((ingredient: Ingredient): string => {
    ...
    <h2 class="text-xl font-bold"> Préparation </h2>
        <ul class='flex flex-col gap-2'>
        ${recipe.steps.map((step: { name: string; quantity: string }): string => {
            return '<li>- ' + step + '</li>'
        }).join('')
        }
            </ul>
